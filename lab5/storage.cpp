...
vector<Language> Storage::getAllLanguages(void) {
    return this -> lang_;
}

bool Storage::updateLanguage(const Language & language) {
...
bool Storage::removeLanguage(int language_id) {
    int index = -1;
...
int Storage::insertLanguage(const Language & language) {
...
optional<Language> Storage::getLanguageById(int language_id) {
    for (Language & l : this -> lang_) {
        if (l.id == language_id) {
            return l;
        }
...
string Storage::readAllFromFile (string const & filename) {
    ifstream file;
...
void Storage::writeAllToFile (string const & filename, string const & text) {
    ofstream file;
    file.open(filename);
    if (!file.is_open()) {
...
vector<Programmer> Storage::getAllProgrammer(void) {
    return this -> lang2_;
}

bool Storage::updateProgrammer(const Programmer & programmer) {
...
bool Storage::removeProgrammer(int programmer_id) {
    int index = -1;
...
int Storage::insertProgrammer(const Programmer & programmer) {
...
optional<Programmer> Storage::getProgrammerById(int programmer_id) {
    for (Programmer & l : this -> lang2_) {
